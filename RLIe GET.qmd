...
The RLIe is often calculated using a relatively small and uneven number of ecosystem types within a group or ecosystem classification level. Because the index is sensitive to ecosystem composition—both in terms of the number of types assessed and variation in threat status categories—the resulting RLIe values are subject to uncertainty and are not perfectly precise. Consequently, small changes in the set of assessed ecosystems can lead to noticeable changes in the index score.

To quantify this uncertainty, non-parametric bootstrap resampling was applied. For each bootstrap iteration, ecosystems within each group or classification level were sampled with replacement to generate a new dataset of the same size as the original group. As a result, some ecosystems may appear more than once in a given resample, while others may be omitted, although the overall sample size is retained. The RLIe was then recalculated for each resampled dataset, and this process was repeated n times to generate a distribution of plausible RLIe values. From this distribution, the 2.5th and 97.5th percentiles were extracted to derive a 95% confidence interval around each RLIe estimate. 
