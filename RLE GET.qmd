...
#  -------1. Distinct IUCN GET 2025 Level 2 functional groups occur in each Realm, by RLE 2025 category -------
GET_L2.count <- EDD.GET_names %>%
                distinct(IUCN_GET_2025_L2, Realm, RLE_2025) %>%
                count(Realm, RLE_2025) %>%
                tidyr::pivot_wider(names_from = RLE_2025,
                                   values_from = n,
                                   values_fill = 0) %>%
                mutate(Total = rowSums(select(., -Realm)))  %>%
                rowwise() %>%
...
GET_L3.count <- EDD.GET_names %>%
                distinct(IUCN_GET_2022_L3, Realm, RLE_2025) %>%
                count(Realm, RLE_2025) %>%
                tidyr::pivot_wider(names_from = RLE_2025,
                                   values_from = n,
                                   values_fill = 0) %>%
